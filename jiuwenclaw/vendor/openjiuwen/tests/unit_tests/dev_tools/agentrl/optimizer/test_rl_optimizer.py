# -*- coding: utf-8 -*-
"""Unit tests for RLOptimizer (run_name, persistence, metrics, agent_factory, reward, config)."""
...
# RLOptimizer and task_runner require ray (and ray._private), hydra, omegaconf
...
def test_init_run_name_contains_experiment_and_timestamp(minimal_config):
    with patch("openjiuwen.dev_tools.agentrl.optimizer.rl_optimizer.datetime") as mdt:
        mdt.now.return_value.strftime.return_value = "20260304_120000"
        opt = RLOptimizer(minimal_config)
...
def test_set_tools_and_resolve_agent_factory_returns_factory(minimal_config):
    ...
    with patch(
        "openjiuwen.dev_tools.agentrl.optimizer.rl_optimizer.build_agent_factory"
    ) as m:
