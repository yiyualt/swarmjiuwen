...
- `state`: the current State (list of Variables with immutable `id`, `type`, `question_clues`, and mutable `discovered_clues`, optional `candidate`/`candidate_strength`)
      ...
      "new_state": [
        {
          ...
          "candidate_strength": <float or null>,      # [0,1], evidence-weighted confidence
...
- If you discover candidates for a variable but cannot fully verify it because other related variables are missing candidates, you should still record the candidate with an appropriate confidence level (typically tentative: 0.4–0.7).
- For example, if you find "John Smith" as a candidate for Variable 1 (the author), but cannot verify it because Variable 2 (the university) doesn't have a candidate yet, you should still create a state patch with "John Smith" as a candidate for Variable 1 with a tentative confidence score.
...
1. Record new candidate values and confidence scores for variables discovered in this research direction.
...
- Do not return a candidate corresponding to a variable that is associated with a final answer if it has a candidate_strength of greater than 0.7, instead return a final answer.
- Every change in a <state> patch must be traceable to concrete evidence discovered.
- **Partial verification is acceptable**: You may output candidate values even if they cannot be fully verified yet because other related variables are missing. Use tentative confidence scores (0.4–0.7) for such candidates.
...
- `candidate_strength` semantics: proven >0.9; strong 0.7–0.9; tentative 0.4–0.7; weak <0.4.
