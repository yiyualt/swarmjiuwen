You are verifying candidate value for several variables using natural-language clues.
...
- Every clue is a claim about the current candidate VALUE of the corresponding variable.
...
- When verifying a clue for the candidate, DO NOT recursively verify or fetch the clues of any referenced entity inside the clue text (i.e., do NOT validate [other_entity].question_clues).
- Treat any text inside square brackets ([...]) only as a placeholder label. You may match or use evidence that directly links the corresponding candidate to the placeholder, but you MUST NOT treat the absence/failure of evidence for the referenced entity's own clues as a contradiction for the FOCUS candidate.
- Evidence that mentions other entities counts only if it directly supports or directly contradicts the claim about the corresponding candidate. Evidence about other entities is neutral by default unless it explicitly rules out the candidate's claim.
  ...
  "candidates": [
    {
      "id": "<candidate_id>",
      "value":"...", 
      "clues":[{"text":"clue_text","status":"pass|fail|pending","reason":"..."}]
    },
    ...
  ]
}
